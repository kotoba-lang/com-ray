(ns ray.main
  "Kotodama WASM entrypoint for the ray clean-room actor (L4) — Clojure port.
  ...
  facts (`ray.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ray.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ray-compat" :tier tier :entities entities} 200])
